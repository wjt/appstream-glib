...
static const struct {
	...
	const gchar	*desc_mild;  /* for %AS_CONTENT_RATING_VALUE_MILD */
	const gchar	*desc_moderate;  /* for %AS_CONTENT_RATING_VALUE_MODERATE */
	const gchar	*desc_intense;  /* for %AS_CONTENT_RATING_VALUE_INTENSE */
} oars_descriptions[] = {
	{ "violence-cartoon",
	...
	N_("Prolonged nudity"),
	/* TRANSLATORS: content rating description */
	N_("Explicit nudity showing nipples or sexual organs") },
	...
	/* TRANSLATORS: content rating description */
	N_("Users are encouraged to donate real money"), /* v1.1 */
	NULL, /* MODERATE */
	/* TRANSLATORS: content rating description */
	...
	/* TRANSLATORS: content rating description */
	N_("Checking for the latest application version"), /* v1.1 */
	/* TRANSLATORS: content rating description */
	N_("Sharing diagnostic data that does not let others identify the user"), /* v1.1 */
	/* TRANSLATORS: content rating description */
...
/**
 * as_content_rating_attribute_to_description:
 * @id: the subsection ID e.g. `violence-cartoon`
 * @value: the #AsContentRatingValue, e.g. %AS_CONTENT_RATING_VALUE_INTENSE
 *
 * Gets a localized description of a specific rating level.
 *
 * Returns: The description, or %NULL if no description is available.
 *
 * Since: 0.7.16
 **/
const gchar *
as_content_rating_attribute_to_description (const gchar          *id,
					    AsContentRatingValue  value)
{
	if (value == AS_CONTENT_RATING_VALUE_UNKNOWN ||
	    value == AS_CONTENT_RATING_VALUE_LAST)
	...
	for (gsize i = 0; i < G_N_ELEMENTS (oars_descriptions); i++) {
		if (g_strcmp0 (id, oars_descriptions[i].id) == 0) {
			switch (value) {
			case AS_CONTENT_RATING_VALUE_NONE:
				return gettext (oars_descriptions[i].desc_none);
			case AS_CONTENT_RATING_VALUE_MILD:
				return gettext (oars_descriptions[i].desc_mild);
			case AS_CONTENT_RATING_VALUE_MODERATE:
				return gettext (oars_descriptions[i].desc_moderate);
			case AS_CONTENT_RATING_VALUE_INTENSE:
				return gettext (oars_descriptions[i].desc_intense);
			case AS_CONTENT_RATING_VALUE_UNKNOWN:
			case AS_CONTENT_RATING_VALUE_LAST:
			default:
				/* Handled above. */
